// @flow
import 'raf/polyfill';
import createInfernoContext, { type Context } from '../implementation';
...
const ThemeContext: Context<Theme> = createInfernoContext('light');
...
test('can skip consumers with bitmask', () => {
  ...
  const Context = createInfernoContext({ foo: 0, bar: 0 }, (a, b) => {
    let result = 0;
    if (a.foo !== b.foo) {
      result |= 0b01;
    }
